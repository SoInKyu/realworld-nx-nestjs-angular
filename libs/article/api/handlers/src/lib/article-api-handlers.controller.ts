...
export class ArticleApiHandlersController {
  ...
  async findAllFeed(@Req() req, @Query() query): Promise<IResponse<IArticle>> {
    ...
    return new ListSuccessResponse<IArticle>({
      listData: await this.mapToResponseArticles(req?.user?.sub, res),
  ...
  async findAll(@Req() req, @Query() query): Promise<IResponse<IArticle>> {
    ...
    if (favorited) {
      ...
      if (user) {
        ...
        return new ListSuccessResponse<IArticle>({
          listData: await this.mapToResponseArticles(jwtInfo?.sub, res),
          total: count,
        });
      }
    }
  ...
  async findAllComments(
    ...
    return new ListSuccessResponse<IComment>({
      listData: await this.mapToResponseComments(jwtInfo?.sub, res),
  ...
  private async mapToResponseArticles(
    requestUserId: string,
    articles: Article[]
  ): Promise<IArticle[]> {
    if (!articles.length) {
    ...
    const authorIds = [
      ...new Set(articles.map((a) => a.authorId).filter(Boolean)),
    ];
    const slugs = [...new Set(articles.map((a) => a.slug).filter(Boolean))];

    const [authors, userFavorites, countResults] = await Promise.all([
      this.userService.findAll({ where: { id: In(authorIds) } }),
      requestUserId && slugs.length
        ? this.favoriteService.findAll({
            where: { userId: requestUserId, articleSlug: In(slugs) },
          })
        : Promise.resolve([]),
      slugs.length
        ? this.favoriteService.repository
            .createQueryBuilder('favorite')
            .select('favorite.articleSlug', 'articleSlug')
            .addSelect('COUNT(*)', 'count')
            .where('favorite.articleSlug IN (:...slugs)', { slugs })
            .andWhere('favorite.deletedDate IS NULL')
            .groupBy('favorite.articleSlug')
            .getRawMany()
        : Promise.resolve([]),
    ]);
    ...
    const favoritedSlugs = new Set(
      (userFavorites as any[]).map((f: any) => f.articleSlug)
    );
    const countMap = new Map(
      (countResults as any[]).map((r: any) => [
        r.articleSlug,
        parseInt(r.count, 10),
      ])
    );
    ...
    return Promise.all(
      articles.map(async (article) => ({
        ...article,
        favorited: favoritedSlugs.has(article.slug),
        favoritesCount: countMap.get(article.slug) || 0,
        author: await this.userService.getProfile(
          requestUserId,
          authorMap.get(article.authorId)
        ),
      }))
    );
  }

  private async mapToResponseComments(
    requestUserId: string,
    comments: Comment[]
  ): Promise<IComment[]> {
    if (!comments.length) {
    ...
    const authorIds = [
      ...new Set(comments.map((c) => c.authorId).filter(Boolean)),
    ];
    const authors = await this.userService.findAll({
